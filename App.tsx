...
type Inputs = {
  ...
  repeatRateUpliftPct: number
  basketUpliftPct: number
  walletShareShiftPct: number
...
const defaultInputs: Inputs = {
  ...
  membersCount: 8000,
  annualSpendPerMember: 7000,
  baselineChurnPct: 0.18,
  repeatRateUpliftPct: 0.06,
  basketUpliftPct: 0.03,
  walletShareShiftPct: 0.04,
  churnReductionPct: 0.10,
  trustIndex: 0.7,
}
...
function irr(cfs:number[]){
  const same=cfs.every(c=>c>=0)||cfs.every(c=>c<=0); if(same) return NaN
  let r=0.1
  for(let k=0;k<100;k++){
    let f=0,df=0
    for(let t=1;t<=cfs.length;t++){
      const c=cfs[t-1]
      f += c/Math.pow(1+r,t)
      df += (-t*c)/Math.pow(1+r,t+1)
    }
    const rn = r - f/df
    if(!isFinite(rn) || Math.abs(rn-r)<1e-7){ r=rn; break; }
    r = rn
  }
  return r
}

function computeBaseline(inp: Inputs){
  const revenue = inp.annualRevenuePerStore
  const gp = revenue * inp.grossMarginPct
  const cogs = revenue - gp
  const labor = inp.laborCostPctOfRevenue * revenue
  const shrink = inp.shrinkPctOfCOGS * cogs
  const op = gp - labor - shrink
  return { revenue, gp, cogs, labor, shrink, op }
}

function loyaltyUpliftRevenue(inp: Inputs, scale: number){
  const membersRevenue0 = Math.min(
    inp.annualRevenuePerStore,
    inp.membersCount * inp.annualSpendPerMember
  )
  const trustF = 0.5 + 0.5 * inp.trustIndex
  const L =
    (1 + inp.repeatRateUpliftPct * trustF) *
    (1 + inp.basketUpliftPct * trustF) *
    (1 + inp.walletShareShiftPct * trustF) - 1
  const churnUplift =
    membersRevenue0 * inp.baselineChurnPct * inp.churnReductionPct * trustF
  const loyaltyExtra = (membersRevenue0 * L + churnUplift) * scale
  return { loyaltyExtra, membersRevenue0, trustF }
}

function computeWithOrb(inp: Inputs, scale: number){
  const opUplift = inp.annualRevenuePerStore * inp.salesUpliftPct * scale
  const { loyaltyExtra } = loyaltyUpliftRevenue(inp, scale)
  const revenue = inp.annualRevenuePerStore + opUplift + loyaltyExtra
  const gm = Math.min(0.99, Math.max(0, inp.grossMarginPct + inp.gmImprovementPP * scale))
  const gp = revenue * gm
  const cogs = revenue - gp
  const labor = inp.laborCostPctOfRevenue * revenue * (1 - inp.laborEfficiencyPct * scale)
  const shrink = inp.shrinkPctOfCOGS * cogs * (1 - inp.shrinkReductionPct * scale)
  const op = gp - labor - shrink
  return { revenue, gp, cogs, labor, shrink, op, opUplift, loyaltyExtra }
}
...
export default function App(){
  ...
  const baseline = useMemo(()=>computeBaseline(inp), [inp])
  const years = [
    { name:'År 1', scale: inp.rampY1 },
    { name:'År 2', scale: inp.rampY2 },
    { name:'År 3', scale: inp.rampY3 },
  ]

  const perStoreRows = years.map((y)=>{
    const w = computeWithOrb(inp, y.scale)
    const compliance = inp.complianceSavingsPerStore * y.scale
    const fee = inp.feePerStorePerYear
    const incremental = (w.op - baseline.op) + compliance - fee
    return { year: y.name, scale: y.scale, ...w, baselineOp: baseline.op, compliance, fee, incremental }
  })

  const cf = perStoreRows.map(r=>r.incremental)
  const storeNPV = npv(inp.discountRate, cf)
  const storeIRR = irr(cf)
  const fullBenefitIncremental =
    (computeWithOrb(inp, 1.0).op - baseline.op) +
    inp.complianceSavingsPerStore - inp.feePerStorePerYear
  const roi = inp.feePerStorePerYear !== 0 ? fullBenefitIncremental / inp.feePerStorePerYear : NaN
  const payback = fullBenefitIncremental > 0 ? inp.feePerStorePerYear / fullBenefitIncremental : NaN
  ...
  const chartData = years.map((y) => {
    ...
    return {
      name: y.name,
      'Operasjonell uplift': Math.round(w.opUplift * factor),
      'Lojalitet & tillit':  Math.round(w.loyaltyExtra * factor),
    }
  })
  ...
  const scenarioRows = scenarios.map((key)=>{
    ...
    const yrows = years.map(y=>{
      const w = computeWithOrb(s, y.scale)
      const comp = s.complianceSavingsPerStore * y.scale
      const fee = s.feePerStorePerYear
      return (w.op - b.op) + comp - fee
    })
    const fb = (computeWithOrb(s,1.0).op - b.op) + s.complianceSavingsPerStore - s.feePerStorePerYear
  ...
  const y1OpRev = inp.annualRevenuePerStore * inp.salesUpliftPct * inp.rampY1
  const y1LoyRev = loyaltyUpliftRevenue(inp, inp.rampY1).loyaltyExtra
          ...
          <>
            <div className="header">
              ...
              <div className="row">
                ...
                <button className="btn" onClick={()=>downloadCSV(perStoreRows)}>Export CSV</button>
            ...
            <div className="grid" style={{gridTemplateColumns:'1.3fr 1fr', marginTop:14}}>
              <div className="card">
                <h3 className="section-title">Attribusjon av omsetningsuplift per butikk ({monthly ? 'per måned' : 'per år'})</h3>
                <div style={{width:'100%', height:360}}>
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={chartData}>
                      <XAxis dataKey="name" stroke="#9aa8c7" />
                      <YAxis stroke="#9aa8c7" />
                      <Tooltip formatter={(v: any) => fmt(Number(v), inp.currency)} />
                      <Legend />
                      <Bar dataKey="Operasjonell uplift" stackId="a" />
                      <Bar dataKey="Lojalitet & tillit"  stackId="a" />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
                <div className="footer">
                  <span className="legend">Discount rate {pct(inp.discountRate)}</span>
                  <span className="legend">Y1 Operasjonell: {fmt(y1OpRev*(monthly?1/12:1), inp.currency)}</span>
                  <span className="legend">Y1 Lojalitet & tillit: {fmt(y1LoyRev*(monthly?1/12:1), inp.currency)}</span>
                </div>
              </div>
              ...
              <div className="card">
                ...
                <div className="controls">
                  ...
                  <div style={{gridColumn:'1 / -1', marginTop:6}}><div className="kpi-label">Lojalitet & tillit</div></div>
                  <div>
                    <label className="label">Aktive medlemmer</label>
                    <input className="input" type="number" value={inp.membersCount} onChange={setNum('membersCount')} />
                  </div>
                  <div>
                    <label className="label">Årlig spend pr medlem ({inp.currency})</label>
                    <input className="input" type="number" value={inp.annualSpendPerMember} onChange={setNum('annualSpendPerMember')} />
                  </div>
                  <div>
                    <label className="label">Repeat uplift (%)</label>
                    <input className="input" type="range" min={0} max={100} step={0.1} value={inp.repeatRateUpliftPct*100} onChange={setPct('repeatRateUpliftPct')} />
                    <div className="legend">{(inp.repeatRateUpliftPct*100).toFixed(1)}%</div>
                  </div>
                  <div>
                    <label className="label">Basket uplift (%)</label>
                    <input className="input" type="range" min={0} max={100} step={0.1} value={inp.basketUpliftPct*100} onChange={setPct('basketUpliftPct')} />
                    <div className="legend">{(inp.basketUpliftPct*100).toFixed(1)}%</div>
                  </div>
                  <div>
                    <label className="label">Wallet share shift (%)</label>
                    <input className="input" type="range" min={0} max={100} step={0.1} value={inp.walletShareShiftPct*100} onChange={setPct('walletShareShiftPct')} />
                    <div className="legend">{(inp.walletShareShiftPct*100).toFixed(1)}%</div>
                  </div>
                  <div>
                    <label className="label">Churn reduksjon (%)</label>
                    <input className="input" type="range" min={0} max={100} step={0.1} value={inp.churnReductionPct*100} onChange={setPct('churnReductionPct')} />
                    <div className="legend">{(inp.churnReductionPct*100).toFixed(1)}%</div>
                  </div>
                  <div>
                    <label className="label">Baseline churn (%)</label>
                    <input className="input" type="range" min={0} max={100} step={0.1} value={inp.baselineChurnPct*100} onChange={setPct('baselineChurnPct')} />
                    <div className="legend">{(inp.baselineChurnPct*100).toFixed(1)}%</div>
                  </div>
                  <div>
                    <label className="label">Trust index (0–1)</label>
                    <input className="input" type="range" min={0} max={1} step={0.01} value={inp.trustIndex} onChange={(e)=>setInp(s=>({...s, trustIndex: Number(e.target.value)}))} />
                    <div className="legend">{inp.trustIndex.toFixed(2)}</div>
                  </div>

                  <div style={{gridColumn:'1 / -1', marginTop:6}}><div className="kpi-label">Adopsjon</div></div>
                  <div>
        ...
        {tab==='assumptions' && (
          <div className="card">
            ...
            <ul className="legend">
              ...
              <li>Members: {inp.membersCount.toLocaleString('no-NO')} × {fmt(inp.annualSpendPerMember, inp.currency)} pr år</li>
              <li>Trust index: {inp.trustIndex.toFixed(2)} → modulerer loyalty-effekt</li>
            </ul>
          </div>
        )}
        ...
        {tab==='about' && (
          <div className="card">
            ...
            <p className="legend">
              ...
              Simulatoren viser NPV/IRR/ROI for en kjede med {inp.stores} butikker, inkludert både
              operasjonelle effekter (salg/margin/arbeid/svinn) og lojalitet & tillit (members, repeat, basket, wallet, churn).
            </p>
          </div>
        )}
      </main>
...
function downloadCSV(rows:any[]){
  ...
  a.href = url; a.download = "orbital_roi_per_store.csv"; a.click()
